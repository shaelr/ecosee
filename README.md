...
| `background_color` | no       | Overrides the Home/Standby Screen background: any CSS color (`#1a1a2e`, `rgba(...)`, a named color), or `transparent` for no background. A menu/picker overlay keeps its own opaque canvas regardless (it needs to fully cover the Home Screen to stay readable), tracking a custom color but never `transparent`. Picker/chip text stays legible either way. Default near-black. |
