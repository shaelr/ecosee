...
### Date/time pickers vs. native `<select>`

Editable date/time fields (Furnace Filter's Last Changed, Schedule's Start/End and Start Time) push ecosee's own `date-picker-overlay.ts`/`time-picker-overlay.ts` Overlays (ADR-0018) — a plain `<button>` emits an `ecosee-date-picker-open`/`ecosee-time-picker-open` event, `ecosee-card.ts` pushes the picker onto the nav stack, and its confirm event drives the write at the card level. There is no native `<input type="date">`/`<input type="time">` involved anywhere in this Card anymore — that architecture (a button calling both `.focus()` and `.showPicker()` on a paired hidden input) is retired; ADR-0017/0018 document why it was replaced (irreconcilable cross-browser/cross-engine quirks — a Chrome desktop rendering bug, an unimplemented iOS WebKit API — chased across five corrections before the decision was made to stop relying on native picker chrome at all). Native `<select>` elements are unaffected and keep the original, simpler pattern: a plain `opacity: 0` full-cover `<select>` (`fan-overlay.ts`'s runtime dropdown, Comfort Setting, Interval) — `<select>` never had any of the date/time-specific engine bugs.
